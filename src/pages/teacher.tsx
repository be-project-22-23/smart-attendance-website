...
const columns: ColumnsType<DataType> = [
  {
    ...
    render: (_, record) => (
      <p>
        {record.classYear + "-" + record.classDivision + " " + record.classDept}
      </p>
    ),
    width: "25%",
  },
  ...
  },
  {
    ...
    render: (_, record) => (
      <p>{new Date(record.startTime + record.duration).toLocaleString()}</p>
    ),
  },
  {
    ...
    key: "attendanceCount",
    sorter: (a, b) =>
      (a.students !== null ? a.students.split(",").length : 0) -
      (b.students !== null ? b.students.split(",").length : 0),
    render: (_, record) => (
      <p>{record.students !== null ? record.students.split(",").length : 0}</p>
    ),
  },
  {
    ...
    render: (_, record) => (
      <a
        href={getDownloadUrl(record.attendanceId)}
        className="bg-[#1677ff] text-white px-3 rounded-lg py-2"
      >
        Download Record
      </a>
    ),
...
const Teacher = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((data) => {
        const newData = data.data.data.map((attendance: any) => {
          return {
            attendanceId: attendance.attendanceId,
            class:
              attendance.classYear +
              "-" +
              attendance.classDivision +
              " " +
              attendance.classDept,
            subject: attendance.classSubject,
            startTime: new Date(attendance.startTime).toLocaleTimeString(),
            attendanceCount:
              attendance.students !== null
                ? attendance.students.split(",").length
                : 0,
          };
        });
  ...
  useEffect(() => {
    const newDisplayAttendance = attendances.filter((attendance: any) => {
      return (
        ...
        attendance.startTime + attendance.duration <= endTime
      );
    });
    setDisplayAttendance(newDisplayAttendance);
  }, [startTime, endTime]);
  ...
  return (
    <div className="overflow-x-hidden">
      <Head>
        <title>
          {teacherDetails !== null ? teacherDetails?.name : "Teacher"} - Present
          Ma'am
        </title>
      </Head>
      <div className="flex items-center mt-5 justify-between px-5">
        <p className="font-bold text-center flex-1">
          Hey, {teacherDetails?.name}! Welcome to Present Ma'am
        </p>
        <Button
          onClick={() => {
      ...
      <nav className="p-5">
        <Select
          ...
          defaultValue={["FE", "SE", "TE", "BE"]}
          onChange={handleClassYearChange}
          options={defaultClassYear}
        />
        <Select
          mode="multiple"
          allowClear
          ...
          className="ml-2"
          placeholder="Please select the departent"
          defaultValue={[
